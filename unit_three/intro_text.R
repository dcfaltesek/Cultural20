#look over some basic structure

#textfeatures
#tidytext
#quanteda - corpus
#mallet - imports from a tidy-like structure
#keras - not doing this because we don't want to break, this relies on a padded matrix like strucutre

#this is a sparse and useful package
library(textfeatures)

#for documentation see ??count_functions

features<-textfeatures(today$Text, sentiment=FALSE)
featured_today<-bind_cols(today, features)

library(ggplot2)
ggplot(featured_today, aes(n_uq_words, n_prepositions, size=n_commas, colour=Source))+geom_jitter()
...
#this is much more useful when working with stories....

#let's do this again with something more fun...
