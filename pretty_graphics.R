...
ggplot(carrier_flights)+geom_bar(mapping = aes(x = day, fill=carrier))+scale_fill_brewer(palette = "Greens")+coord_polar()

#diamonds
ggplot(diamonds)+geom_bar(mapping = aes(x = price, fill=cut))+scale_fill_brewer(palette = "Greens")+coord_polar()
...
ggplot(patty, aes(carrier, origin))+geom_jitter(patty, mapping=aes(color=dest))+scale_colour_brewer(palette = "Set2")

#now lets manipulate the color there
ggplot(patty, aes(carrier, origin, color=dep_delay))+scale_fill_gradient()+geom_jitter()+scale_colour_gradient2_tableau(palette = "Orange-Blue Diverging", na.value = "grey50", guide = "colourbar")
ggplot(patty, aes(carrier, origin, color=dep_delay))+geom_jitter()+scale_colour_gradient2()
